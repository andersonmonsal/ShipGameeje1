...
def shoot():
    """Ruta para realizar un disparo y cambiar de turno"""
    try:
        # Obtiene position como cadena
        position = request.form['position']
        if not isinstance(position, str):
            position = str(position)

        # Procesamiento de la posición (columnas A-J, filas 1-10)
        column = position[0].upper()
        row = int(position[1:]) - 1
        column_map = {'A': 0, 'B': 1, 'C': 2, 'D': 3, 'E': 4, 'F': 5, 'G': 6, 'H': 7, 'I': 8, 'J': 9}
        
        if column not in column_map or row < 0 or row > 9:
            return jsonify({'success': False, 'message': 'Posición no válida'})

        col_index = column_map[column]
        result = game_controller.realizar_disparo((row, col_index))
        response = {
            'success': True,
            'hit': result,
        ...
        if response['game_over']:
            ...
            # Cambia de turno si el juego no ha terminado
            game_controller.cambiar_turno()
            response['current_player'] = game_controller.juego.currentplayer.nombre
...
def play_move():
    """Ruta para realizar un movimiento en el juego"""
    try:
        position = request.form['position']
        if game_controller.realizar_disparo(position):
            return jsonify({'success': True, 'hit': True})
        else:
...
def current_player():
    """Ruta para obtener el jugador actual"""
    try:
        player = game_controller.juego.currentplayer
        return jsonify({'nombre': player.nombre})
    except Exception as e:
        print(f"Error al obtener jugador actual: {e}")
        return jsonify({'error': str(e)}), 500
